...
namespace WBSF
{
	...
	public:
		...
		bool IsLast()const{ assert(m_no >= 0 && m_no < m_total); return m_no == m_total - 1; }
		size_t GetNo()const{ assert(m_no >= 0 && m_no < m_total); return m_no; }
	...
	public:
		...
		CTRef GetTRef(const CTRef& TRef)const{ return GetTRef(TRef.GetYear()).as(TRef.TM()); }
