...
{
    assert(m_SetNormalDBFilePath != NULL);
...
{
    assert(m_SetDailyDBFilePath != NULL);
...
//****************************************************************************
ERMsg CTempGenHelp::Save(const std::vector<std::string>& outputFilePathVector)const
{
    assert(m_Save != NULL);
