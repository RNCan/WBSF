...
{
    return ((CDHDatabaseBase&)CHourlyDatabase()).GetVersion(filePath);
}
ERMsg CHourlyDatabase::DeleteDatabase(const std::string&  outputFilePath, CCallback& callback)
{
    return ((CDHDatabaseBase&)CHourlyDatabase()).DeleteDatabase(outputFilePath, callback);
}
...
{
    return ((CDHDatabaseBase&)CHourlyDatabase()).RenameDatabase(inputFilePath, outputFilePath, callback);
}
